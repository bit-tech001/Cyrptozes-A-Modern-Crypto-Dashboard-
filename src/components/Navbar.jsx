    ...
    <nav className='flex flex-wrap md:flex-nowrap items-center justify-between gap-4 px-[5%] md:px-[8%] lg:px-[10%] py-5 bg-gray-900/80 backdrop-blur-md border-b border-gray-700/30 sticky top-0 z-50'>
      ...
      <form onSubmit={searchHandler} className='order-3 w-full md:order-2 md:w-auto flex-1 max-w-2xl mx-0 md:mx-4 relative'>
        <div className='relative'>
          ...
          />
          <button type='submit' className='z-10 absolute right-2 top-1/2 -translate-y-1/2 px-4 py-1.5 bg-gradient-to-r from-emerald-600 to-cyan-600 text-white rounded-full hover:scale-105 transition-all'>
            <Search className='w-4 h-4 pointer-events-none' />
          </button>
        </div>

        {filteredCoins.length > 0 && (
          <ul className='absolute w-full bg-gray-800/95 border border-gray-600 mt-2 rounded-lg shadow-xl z-20 backdrop-blur-md'>
